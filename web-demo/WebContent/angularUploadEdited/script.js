		...
		app.directive("imgUpload",function($http,$compile){
			return {
				...
				link : function(scope,elem,attrs){
					...
					elem.find('.fileUpload').on('change',function(e){
						uploadFile(e,'formControl');
					});

					elem.find('.dropzone').on("click",function(e){
						$compile(elem.find('.fileUpload'))(scope).trigger('click');
					});

					elem.find('.dropzone').on("dragover",function(e){
						e.preventDefault();
					});

					elem.find('.dropzone').on("drop",function(e){
						uploadFile(e,'drop');																		
					});
